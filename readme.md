...
- **配置管理**：在线修改系统配置
- **缓存管理**：查看和清理媒体缓存
- **图片管理**：查看、筛选、导出、删除生成的图片，支持质量分析和自动筛选

### 图片管理功能

访问地址：`http://<host>:8000/gallery`

**核心功能**：
- 📊 **统计信息**：图片总数、总大小、本月新增、常用标签
- 🔍 **筛选功能**：按提示词、模型、宽高比、质量评分筛选
- 📈 **质量分析**：自动检测模糊度、亮度、对比度，生成质量评分
- 🤖 **自动删除**：图片生成后自动质量检测，低于30分自动删除
- 🏷️ **标签管理**：添加、编辑、删除图片标签
- 📤 **批量操作**：批量导出ZIP、批量删除、批量质量分析
- 📁 **本地同步**：扫描本地图片文件夹，自动生成元数据
- ⬆️ **图片上传**：支持点击上传和拖拽上传
- 🎲 **随机查看**：加权随机浏览图片，支持收藏、删除、下载等快捷操作
...
- 对比度评分（0-100）
- 综合质量评分（加权平均）
- 质量问题标记（极度模糊、严重模糊、极度过暗/过亮、极低对比度）
